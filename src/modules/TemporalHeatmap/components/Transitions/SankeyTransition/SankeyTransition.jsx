    ...
    drawTransitions() {
        ...
        this.props.firstGrouped.forEach((sourcePartition, i) => {
            sourceProxyPositions.push({
                ...
                sharedWidth: this.props.visStore.groupScale(sourcePartition.patients.filter(patient => [].concat(...this.props.secondGrouped.map(partition => partition.patients)).includes(patient)).length),
            ...
            this.props.secondGrouped.forEach((targetPartition, j) => {
                if (i === 0) {
                    targetProxyPositions.push({
                        key: targetPartition.partition,
                        x0: targetPartitionPos,
                        sharedWidth: this.props.visStore.groupScale(targetPartition.patients.filter(patient => [].concat(...this.props.firstGrouped.map(partition => partition.patients)).includes(patient)).length),
                ...
                if (patientIntersection.length !== 0) {
                    ...
                    if (selectedIntersection.length > 0) {
                        ...
                        sourceProxyPositions[i].selected.push([currXsource, currXsource + selectedWidth]);
                        targetProxyPositions[j].selected.push([currXtarget[targetPartition.partition], currXtarget[targetPartition.partition] + selectedWidth]);
                    }
