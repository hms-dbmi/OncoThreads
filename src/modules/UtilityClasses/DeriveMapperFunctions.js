...
class DerivedMapperFunctions {
		...
		switch (modification.type) {
			...
			default: {
				...
				if (modification.transformFunction) {
					intermedMapper = DerivedMapperFunctions.createContinuousTransformMapper(
						mappers[0],
				...
				if (modification.binning) {
					mapper = DerivedMapperFunctions.createBinnedMapper(
						intermedMapper,
						modification.binning.bins,
						modification.binning.binNames
					);
				} else {
	...
	static createContinuousCombinedMapper(mappers, operation) {
		...
		switch (operation) {
			case 'average':
				Object.keys(mappers[0]).forEach((entry) => {
					const filteredValues = mappers
						.map((mapper) => mapper[entry])
