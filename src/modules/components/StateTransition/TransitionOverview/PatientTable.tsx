	...
	getPatientTable() {
		...
		const patientRelatedVars = currentVariables.filter(
			(id: string) =>
				...
				referencedVariables[id].originalIds.every((d: string) => patientVars.includes(d))
		);
		...
		const columns: ColumnsType<RowRecordType> = patientGroups.map((patients, groupIdx) => {
			...
			const title =
				getTextWidth(`group_${groupIdx + 1}`, 12) > cellWidth ? groupIdx + 1 : `group_${groupIdx + 1}`;
			...
			return {
				...
				render: (values, fulldata) => {
					...
					const { key } = fulldata;
					const { domain, name } =
						this.props.rootStore!.dataStore.variableStores.sample.referencedVariables[key];
					...
					return (
						<svg width={cellWidth} height={cellHeight + textHeight}>
							...
							{getTextWidth(name, 12) <= cellWidth ? (
								<text x={cellWidth / 2} y={cellHeight + textHeight} textAnchor="middle">
									{name}
								</text>
							) : (
								<></>
		...
		return <Table bordered columns={columns} dataSource={tableData} pagination={false} className="patientTable" />;
	}
	render() {
