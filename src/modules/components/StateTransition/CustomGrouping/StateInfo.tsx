...
const StateInfo = observer(
	class StateInfo extends React.Component<Props, {}> {
		render() {
			...
			const content = states.map((state, i) => {
				...
				const values = Object.keys(domains).map((key) => {
					...
					return (
						<span style={{ marginRight: '2px' }} key={key}>
				...
				// }
				return (
					<p key={`state_${i}`} style={{ color: getColorByName(stateKey) }}>
						{values}
						<Button
							size="small"
							onClick={() => this.props.deleteGroup(stateKey)}
			...
			return (
				<div
					className="StateInfo"
					style={{ height: this.props.height, paddingTop: '5px', borderTop: '#ccc solid 1px' }}
				...
				>
					...
					<div className="button group" style={{ float: 'right' }}>
						<Button size="small" onClick={this.props.resetGroup}>
							Reset
						</Button>
