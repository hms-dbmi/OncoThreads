...
class StateBlock extends React.Component<Props> {
	...
	get nameColWidth(): number {
		...
		const scoreWidth = getTextWidth((0.0).toFixed(this.scoreDigits) + '  X', this.fontHeight);
	...
	get cellWidth(): number {
		...
		const cellWidth = (width - this.nameColWidth - this.wholeHorizonGap - this.rightMargin) / points.length;
		return cellWidth;
	}
	...
	drawAllStates() {
		...
		const allStates = (
			<g className="state" key="allStates" transform={`translate(${this.nameColWidth}, 0)`}>
				{stateBlocks}
			</g>
		);
	...
	drawOneState(points: IPoint[], stateKey: string) {
		...
		const block = this.drawBlock(points);
	...
	drawBlock(points: IPoint[]) {
		...
		const { setHoverID, resetHoverID } = this.props;
		const block = points.map((point, i) => {
			const pointCol = point.value.map((v, rowIdx) => {
				const fill = this.props.colorScales[rowIdx](v) || 'gray';
				return (
					<rect
						key={rowIdx}
			...
			const opacity = this.props.hoverPointID === point.idx ? 1 : 0.5;

			return (
				<g
					key={`point_${point.idx}`}
					className={`point_${point.idx}`}
					opacity={opacity}
	...
	drawTimeDist(points: IPoint[], stateKey: string) {
		...
		return (
			<g
				className="timeDist"
				key="timeDist"
				transform={`translate(0, ${this.cellHeight * this.attrNum + this.verticalGap})`}
			>
	...
	featureNameRows() {
		...
		const rows = importanceScores.map((d, i) => {
			const { score, name } = d;
			const cropName = cropText(name, this.fontHeight, 400, this.maxNameColWidth);
			...
			return (
				<g key={name} transform={`translate(0, ${this.cellHeight * (i + 1)})`}>
					{featureNameComponent}
					<text
						x={this.nameColWidth - this.strokeW * 2}
						textAnchor="end"
						cursor="pointer"
						onClick={() => {
		...
		return (
			<g
				...
				transform={`translate(${0}, ${this.fontHeight - this.strokeW})`}
	...
	render() {
		const legendLabelTransform = `translate(${this.props.width - this.rightMargin * 0.5}, ${(this.cellHeight * this.attrNum) / 2 + this.fontHeight}) rotate(-90, 0, 0) `;
		const legendLabelWidth = getTextWidth('legend V', this.fontHeight) + 20,
			legendLabelHeight = this.fontHeight * 1.3;
		return (
			<g className="stateSummary" key="stateSummary">
				{this.drawAllStates()}
				<g className="featureLegend labelButton" transform={legendLabelTransform}>
					<rect
						className="legendLabelBackground"
						width={legendLabelWidth}
						height={legendLabelHeight}
						x={-0.5 * legendLabelWidth}
						y={-0.9 * this.fontHeight}
						fill="white"
						stroke="gray"
						rx="2"
					/>
					...
					<Popover placement="right" content={<FeatureLegend cellHeight={this.cellHeight} />} trigger="click">
						<text cursor="pointer" textAnchor="middle">
							legend v
						</text>
					</Popover>
				</g>
