...
class FeatureLegend extends React.Component<Props> {
			...
			return (
				<g key={variableName}>
					...
					<rect
						...
						height={lineheight}
						fill={`url(#gradient_${variableName})`}
					/>
					{text}
				</g>
			);
	...
	getLegend() {
		...
		const varWidth = Math.max(
			...dataStore.variableStores.sample.currentVariables.map((varName: string) =>
				getTextWidth(varName, adaptedFontSize)
			)
		);

		// return dataStore.currentVariables
		return dataStore.variableStores.sample.currentVariables.map((variableName: string, variableIdx: number) => {
			...
			if (variable.datatype === 'STRING' || variable.datatype === 'ORDINAL') {
				...
				legendEntries = [
					this.getContinuousLegend(variableName, opacity, adaptedFontSize, lineheight, colorScale),
				];
			}
			const leTransform = `translate(0,${variableIdx * this.props.cellHeight})`;

			return (
				<g className="featureLegend" transform={leTransform} key={`${variableName}_${variableIdx}`}>
					<text className="feature name" fontSize={adaptedFontSize} y={adaptedFontSize}>
						{variableName}
					</text>
	...
	render() {
		...
		return (
			<svg width={this.maxWidth} height={height} className="featureLegend">
				{content}
			</svg>
		);
	}
