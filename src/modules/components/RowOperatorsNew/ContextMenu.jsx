...
const ContextMenu = inject('dataStore', 'undoRedoStore')(observer(class ContextMenu extends React.Component {
        ...
        if (this.variable.datatype === 'NUMBER' && this.props.action !== 'UNGROUP') {
            if (this.props.action === 'GROUP') {
                this.props.openBinningModal(this.variable, (derivedVariable) => {
                    this.variableStore.replaceDisplayedVariable(
                        this.props.clickedVariable, derivedVariable,
                    );
                    this.variableStore.childStore.applyActionToAll(
                        this.localIndex, derivedVariable.id, this.props.action,
                    );
            ...
            } else if (this.variableStore.childStore.atLeastOneGrouped(
                0, this.variableStore.childStore.timepoints.length - 1,
            )) {
                this.props.openBinningModal(this.variable, (derivedVariable) => {
                    this.variableStore.replaceDisplayedVariable(
                        this.props.clickedVariable, derivedVariable,
                ...
                this.variableStore.childStore.applyActionToAll(
                    this.localIndex, this.props.clickedVariable, this.props.action,
                );
                this.props.undoRedoStore.saveTimepointHistory(`APPLY ${this.props.action} TO ALL`, this.props.clickedVariable, this.type, this.props.clickedTimepoint);
            }
        } else {
     ...
     */
    magicSort() {
        this.variableStore.childStore.timepoints[this.localIndex]
            .magicSort(this.props.clickedVariable);
    ...
    render() {
        ...
        return (
            ...
            >
                <Button
                    onClick={() => this.applyActionToPrevious()}
                >
                    {`Apply ${this.props.action} to previous timepoint`}
                </Button>
                <Button
                    onClick={() => this.applyActionToNext()}
                >
                    {`Apply ${this.props.action} to next timepoint`}
                </Button>
                <Button
                    onClick={() => this.applyActionToAll()}
                >
