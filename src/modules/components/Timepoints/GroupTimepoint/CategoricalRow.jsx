 ...
 */
const CategoricalRow = inject(
	'dataStore',
	'uiStore',
	'visStore'
)(
	observer(
		class CategoricalRow extends React.Component {
			constructor() {
				super();
				this.previousOffset = 0;
			}

			static getTooltipLines(value, numPatients, rowPatients) {
				const patientString = numPatients === 1 ? 'patient' : 'patients';
				const line1 = `${value}`;
				const line2 = `${Math.round((numPatients / rowPatients) * 100)}% (${numPatients} ${patientString}) of group`;
				return { line1, line2 };
			}
							...
							<polygon
								...
								onMouseEnter={(e) =>
									this.props.showTooltip(
										e,
										CategoricalRow.getTooltipLines(f.key, f.patients.length, rowPatients).line1,
									CategoricalRow.getTooltipLines(f.key, f.patients.length, rowPatients).line2
									)
								}
						...
						rects.push(
							<rect
								...
								onMouseEnter={(e) =>
									this.props.showTooltip(
										e,
										CategoricalRow.getTooltipLines(row.key, row.patients.length, rowPatients).line1,
									CategoricalRow.getTooltipLines(row.key, row.patients.length, rowPatients).line2
									)
								}
								onMouseLeave={this.props.hideTooltip}
							/>
