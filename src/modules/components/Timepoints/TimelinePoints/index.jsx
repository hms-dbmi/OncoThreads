 ...
 */
const TimelineTimepoint = inject('rootStore')(
	observer(
		class TimelineTimepoint extends React.Component {
			/**
			...
			getAllEvents(variableId, index, array) {
				const current = this.props.rootStore.dataStore.variableStores.between.referencedVariables[variableId];
				if (current.type === 'event') {
					this.props.rootStore.eventTimelineMap[variableId]
						.filter((d) => d.time === index)
			...
			getGlobalTimepointWithTransition() {
				...
				this.props.timepoint.heatmap.forEach((row, i) => {
					...
					if (this.props.timepoint.type === 'between') {
						...
						color = this.props.currentVariables.filter(
							(d) => d.id === this.props.rootStore.dataStore.globalPrimary
						)[0].colorScale;
						rows.push(
							<g key={row.variable + i + globalIndex}>
								<TimelineRow
			...
			filterEvents(variableId, events) {
				...
				if (
					...
					filterMapper = DerivedMapperFunctions.getModificationMapper(
						...
						variable.originalIds.map(
							(d) => this.props.rootStore.dataStore.variableStores.between.getById(d).mapper
						)
					);
				}
			...
			render() {
				return this.getGlobalTimepointWithTransition();
			}
		}
	)
);
TimelineTimepoint.propTypes = {
	...
	currentVariables: PropTypes.arrayOf(
		PropTypes.oneOfType([PropTypes.instanceOf(OriginalVariable), PropTypes.instanceOf(DerivedVariable)])
	),
