 ...
 */
const GlobalTimeline = inject('rootStore')(
	observer(
		class GlobalTimeline extends React.Component {
			...
			getGlobalTransitions() {
				return (
					<Provider dataStore={this.props.rootStore.dataStore} visStore={this.props.rootStore.visStore}>
						<GlobalTransition
				...
				allEventTimepoints.forEach((timepoint) => {
					timepoint.heatmap
						...
						.forEach((eventId) => {
							...
							allEvents[eventId][timepoint.localIndex] = this.getEvents(
								eventId,
			...
			getOverlappingEventsMap(allTimepoints, allEvents) {
				...
				const globalPrimaryRows = allTimepoints
					...
					.map((d) => d.heatmap.find((row) => row.variable === this.props.rootStore.dataStore.globalPrimary));
				...
				Object.keys(allEvents).forEach((eventId) => {
					...
					eventsList.forEach((event) => {
						...
						eventsMapByPatient[eventId][event.patientId] =
							eventsMapByPatient[eventId][event.patientId].concat(event);
					});
				...
				sortedEventIds.forEach((eventId) => {
					Object.keys(eventsMapByPatient[eventId]).forEach((patient) => {
						...
						const didOverlapWithSample = eventsMapByPatient[eventId][patient].find((event) =>
							globalPrimaryRows
								.map((row) => row.data.find((d) => d.patient === patient))
								.find((sampleData) => sampleData && this.isOverlappingEventSample(event, sampleData))
						);
						const didOverlapWithEvents = eventsMapByPatient[eventId][patient].find((event) =>
							overlappingEvents[patient][0].find((eventId2) =>
								eventsMapByPatient[eventId2][patient].find((event2) =>
									this.isOverlappingEventPair(event, event2)
								)
							)
						);
						if (!didOverlapWithSample && !didOverlapWithEvents) {
							...
							while (true) {
								...
								if (
									!this.isLaneBlocked(eventsMapByPatient, overlappingEvents, eventId, patient, index)
								) {
									overlappingEvents[patient][index] = overlappingEvents[patient][index].concat([
										eventId,
									]);
									break;
								}
         ...
         */
			...
			isLaneBlocked(eventsMapByPatient, overlappingEvents, eventId, patient, index) {
				return eventsMapByPatient[eventId][patient].find((event) =>
					overlappingEvents[patient][index].find((eventId2) =>
						eventsMapByPatient[eventId2][patient].find((event2) =>
							this.isOverlappingEventPair(event, event2)
						)
					)
			...
			isOverlappingEventSample(event, sampleData) {
				const sampleY = this.props.rootStore.visStore.timeScale(
					this.props.rootStore.sampleTimelineMap[sampleData.sample]
				);
			...
			isOverlappingEventPair(event1, event2) {
				const eventHeight1 = this.props.rootStore.visStore.timeScale(
					event1.eventEndDate - event1.eventStartDate
				);
				...
				const eventHeight2 = this.props.rootStore.visStore.timeScale(
					event2.eventEndDate - event2.eventStartDate
				);
			...
			getLegend() {
				if (this.props.rootStore.dataStore.variableStores.sample.currentVariables.length > 0) {
					const globalPrimaryName =
						this.props.rootStore.dataStore.variableStores.sample.fullCurrentVariables.filter(
							(d1) => d1.id === this.props.rootStore.dataStore.globalPrimary
						)[0].name;
					...
					return (
						<div>
							<h5>{`${cropText(
								...
								fontSize,
								fontWeight,
								this.state.rowOperatorsWidth - fontSize
							)} Legend`}</h5>
						...
						<div className="view" id="timeline-view">
							<Row>
								...
								<Col xs={1} md={1} style={{ padding: 0, width: 55 }}>
									<GlobalTimeAxis timeValue={this.props.rootStore.timeValue} width={55} />
								</Col>
								<Col xs={9} md={9} style={{ padding: 0, overflow: 'hidden' }}>
									...
									<div ref={this.selectedTab} className="scrollableX">
										<svg width="100%" height={this.props.rootStore.visStore.svgHeight}>
											<g
												transform={`translate(${this.props.rootStore.visStore.timelineRectSize},${this.props.rootStore.visStore.timelineRectSize / 2})`}
											>
												{transitions}
												{timepoints}
											</g>
