 ...
 */
const BlockTextField = observer(
	class BlockTextField extends React.Component {
		/**
		...
		render() {
			const croppedText = BlockTextField.cropText(
				this.props.timepoint.name,
				(this.props.height - 2) / 2,
				this.props.width
			);
			return (
				<g>
					...
					<text
						...
						x={(this.props.width - getTextWidth(croppedText, (this.props.height - 2) / 2)) / 2 + 1}
					>
						{croppedText}
					</text>
					<foreignObject
