 ...
 */
const CategoryCombine = inject('variableManagerStore')(
	observer(
		class CategoryCombine extends React.Component {
			constructor(props) {
				super(props);
			...
			createCategoryStore() {
				...
				let allValues;
				let colorRange;
				const originalMapper = DerivedMapperFunctions.createCategoryCombinedMapper(
			...
			initializeObservable() {
				let name;
				let nameChanged; // name of combined variable
			...
			handleApply() {
				let datatype;
				let mapper = DerivedMapperFunctions.createCategoryCombinedMapper(
					this.props.variables.map((d) => d.mapper)
				);
				...
				mapper = DerivedMapperFunctions.createModifyCategoriesMapper(
					mapper,
					this.categoryStore.categoryMapping
				);
				...
				return (
					<Modal show={this.props.modalIsOpen} onHide={this.props.closeModal}>
						...
						<Modal.Body style={{ minHeight: '400px' }}>
							...
							<Provider categoryStore={this.categoryStore} key="table">
								<CategoricalTable />
							</Provider>
						</Modal.Body>
						<Modal.Footer>
