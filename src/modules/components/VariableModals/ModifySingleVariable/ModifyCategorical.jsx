 ...
 */
const ModifyCategorical = inject(
	'variableManagerStore',
	'rootStore'
)(
	observer(
		class ModifyCategorical extends React.Component {
			constructor(props) {
				super(props);
					...
					this.props.derivedVariable.domain.forEach((d) => {
						Object.keys(this.props.derivedVariable.modification.mapping).forEach((key) => {
							if (this.props.derivedVariable.modification.mapping[key] === d) {
								...
								currentData[currentData.map((entry) => entry.name).indexOf(d)].categories.push(key);
							}
						});
					});
				} else {
