 ...
 */
const TimepointVariableSelector = inject(
	'variableManagerStore',
	'rootStore'
)(
	observer(
		class TimepointVariableSelector extends React.Component {
			geneListString = '';
			...
			createOptions() {
				...
				this.props.variableManagerStore.savedReferences.forEach((d) => {
					...
					const lb = (
						<div style={{ textAlign: 'left' }} key={d}>
							<b>{variable.name}</b>
				...
				this.props.rootStore.clinicalSampleCategories.forEach((d) => {
					const lb = (
						<div style={{ textAlign: 'left' }} key={d.variable}>
							<b>{d.variable}</b>
				...
				return [
					{ label: 'Saved variables', options: savedOptions },
				...
				if (hasData) {
					if (!this.molecularOptions.map((d) => d.id).includes(profile)) {
						this.molecularOptions.push({
							id: profile,
							profile,
							name: this.props.rootStore.availableProfiles.filter(
								(d) => d.molecularProfileId === profile
							)[0].name,
							selected: false,
						});
					}
			...
			searchGenes() {
				...
				const geneList = this.geneListString
					.replace(/(\r\n\t|\n|\r\t)/gm, '')
					.toUpperCase()
					.split(' ');
				...
				const callback = (dataProfiles) => {
					this.props.rootStore.availableProfiles.forEach((d) => {
						if (d.molecularAlterationType === 'MUTATION_EXTENDED') {
							...
							this.updateMolecularCheckBoxOptions(
								d.molecularProfileId,
								dataProfiles.includes(d.molecularProfileId)
							);
						}
					});
				};
				if (this.props.rootStore.isOwnData) {
					...
					this.props.rootStore.molProfileMapping.getDataContainingProfiles(
						geneList,
						callback,
			...
			getAvailableCheckBoxes() {
				...
				if (this.mutationOptions.length > 0) {
					const available = [];
					this.mutationOptions.forEach((d, i) =>
						available.push(
							<Form.Check
								type="checkbox"
				...
				if (this.molecularOptions.length > 0) {
					const available = [];
					this.molecularOptions.forEach((d, i) =>
						available.push(
							<Form.Check
								type="checkbox"
						...
						if (selectedOption.source !== 'derived') {
							variables.push(
								new OriginalVariable(
									...
									[],
									[],
									this.props.rootStore.staticMappers[selectedOption.id],
						...
						<Form horizontal>
							...
							<Form.Group>
								<Col sm={{ span: 1, offset: 11 }}>
									<Button className="pull-right" onClick={this.addVariables} key="add">
										Add
									</Button>
								</Col>
							</Form.Group>
