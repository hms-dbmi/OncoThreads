...
class MolProfileMapping {
    ...
    filterMolecularData(profileId) {
        ...
        // Is panel measured?
        const notInPanel = this.currentIds.filter(entry => !(Object.values(this.currentPanels[profileId]).join().includes(entry.entrezGeneId)));
        if (notInPanel.length > 0) {
            window.alert(`Gene(s) ${notInPanel.map(d => d.hgncSymbol)} not measured in profile ${profileName}`);
        }
        const availableIds = this.currentIds.filter(entry => Object.values(this.currentPanels[profileId]).join().includes(entry.entrezGeneId));
