...
class MolProfileMapping {
	...
	getMutationsProfile(filteredIds, mappingType) {
		...
		filteredIds.forEach((d) => {
			if (!this.rootStore.dataStore.variableStores.sample.isDisplayed(d.entrezGeneId + mappingType)) {
				const containedIds = this.currentMutations.filter(
					(mutation) => mutation.gene.hugoGeneSymbol === d.hgncSymbol
				);
				let domain = [];
				let range = [];
				...
				variables.push(
					new OriginalVariable(
						d.entrezGeneId + mappingType,
						`${d.hgncSymbol}_${mappingType}`,
						datatype,
						`Mutation in ${d.hgncSymbol}`,
						range,
						domain,
						this.createMutationMapping(containedIds, mappingType, d.entrezGeneId),
						mappingType,
						'gene'
					)
				);
		...
		if (this.currentMolecular[profileId].length !== 0) {
			filteredIds.forEach((d) => {
				if (!this.rootStore.dataStore.variableStores.sample.isDisplayed(d.entrezGeneId)) {
					const containedIds = this.currentMolecular[profileId].filter(
						(entry) => entry.entrezGeneId === d.entrezGeneId
					);
					let domain = [];
					let range = [];
					let datatype = 'NUMBER';
					...
					variables.push(
						new OriginalVariable(
							`${d.entrezGeneId}_${profileId}`,
							`${d.hgncSymbol}_${profile.name}`,
							datatype,
							`${profile.name}: ${d.hgncSymbol}`,
							range,
							domain,
							this.createMolecularMapping(containedIds, datatype),
							profileId,
							'molecular'
						)
					);
	...
	filterMolecularData(profileId) {
		...
		// Is panel measured?
		const notInPanel = this.currentIds.filter(
	...
	createMutationMapping(list, mappingType, entrezID) {
		...
		if (mappingType === 'Binary') {
			...
			mappingFunction = (entry) => {
				...
				if (entry !== undefined && 'VAF' in entry) {
					vaf = entry.VAF;
				}
				return vaf;
			};
					...
					if (entries.length > 0) {
						...
						entries.forEach((currEntry) => {
							if ((currEntry.proteinChange || '').toLowerCase() === 'promoter') {
								...
								let simplifiedMutationType = MolProfileMapping.getMutationType(currEntry.mutationType);
								if (simplifiedMutationType !== 'fusion') {
									if (
										simplifiedMutationType !== 'missense' &&
										simplifiedMutationType !== 'inframe' &&
										simplifiedMutationType !== 'other'
									) {
