...
class VariableStore {
	constructor(rootStore, type) {
		...
		observe(this.currentVariables, (change) => {
			if (change.type === 'splice') {
				...
				if (
					this.type === 'sample' &&
					this.currentVariables.length === change.addedCount - change.removedCount
				) {
					this.rootStore.dataStore.setGlobalPrimary(this.currentVariables[0]);
				}
	...
	findNearestReplace(patient, timeIdx, rowIdx) {
		const { timepoints } = this.childStore;
		let beforeTime = timeIdx - 1,
			afterTime = timeIdx + 1,
			v = timepoints[timeIdx].heatmap[rowIdx].data.find((d) => d.patient === patient).value;

		while (v === undefined) {
			let beforeV, afterV;

			if (afterTime < timepoints.length) {
				const afterSample = timepoints[afterTime].heatmap[rowIdx].data.find((d) => d.patient === patient);
			...
			if (beforeTime >= 0) {
				const beforeSample = timepoints[beforeTime].heatmap[rowIdx].data.find((d) => d.patient === patient);
		...
		if (v === undefined) {
			let otherValues = timepoints[timeIdx].heatmap[rowIdx].data
				.map((d) => d.value)
	...
	updateVariableRanges() {
		...
		profileVariables.forEach((variableId) => {
			if (this.referencedVariables[variableId].profile in profileDomains) {
				this.referencedVariables[variableId].changeDomain(
					profileDomains[this.referencedVariables[variableId].profile]
				);
			}
		});
	}
	...
	recursiveSearch(id, variableType) {
		...
		if (this.referencedVariables[id].derived) {
			return this.referencedVariables[id].originalIds
				.map((d) => this.recursiveSearch(d, variableType))
				.includes(true);
		}
