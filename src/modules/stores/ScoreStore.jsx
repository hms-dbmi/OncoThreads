class ScoreStore {
	...
	calculateVScore() {
		...
		const dTypeRet = function (q) {
			return self.rootStore.clinicalSampleCategories.filter((d) => d.id === q)[0].datatype;
		};
			...
			if (dType !== 'NUMBER') {
				...
				for (var j = 0; j < Object.keys(ST).length; j++) {
					...
					for (var k = 0; k < Object.values(ST)[j].length - 1; k++) {
						...
						if (
							iV[Object.values(ST)[j][k]] !== undefined &&
							iV[Object.values(ST)[j][k + 1]] !== undefined &&
							iV[Object.values(ST)[j][k]] !== iV[Object.values(ST)[j][k + 1]]
						) {
							// console.log(Object.values(ST)[j][k]);
	 ...
	 */
	getModVRAcross(dtype, id, input) {
		// input is the same as iV in calculateVScore()
		const ST = this.rootStore.sampleStructure;
	...
	getCategoricalChangeRate(mapper) {
		...
		Object.keys(this.rootStore.sampleStructure).forEach((patient) => {
			this.rootStore.sampleStructure[patient]
	...
	getNumericalChangeRate(mapper, range) {
		...
		Object.keys(this.rootStore.sampleStructure).forEach((patient) => {
			this.rootStore.sampleStructure[patient]
				...
				.forEach((value, i, array) => {
					if (i !== array.length - 1) {
						if (value !== undefined && array[i + 1] !== undefined) {
							allTransitions += 1;
							sumOfChange += Math.abs(value - array[i + 1]);
						}
					}
				});
		});
