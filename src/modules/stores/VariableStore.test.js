...
describe('VariableStore', () => {
	...
	it('creates new variable', () => {
		rootStore.dataStore.variableStores.sample.addVariableToBeDisplayed(
			new OriginalVariable('id1', 'name1', 'datatype1', 'description1', [], [], {}, '', '')
		);
		rootStore.dataStore.variableStores.sample.addVariableToBeDisplayed(
			new OriginalVariable('id2', 'name2', 'datatype2', 'description2', [], [], {}, '', '')
		);
	...
	it('removes variables that are no longer referenced', () => {
		...
		rootStore.dataStore.variableStores.sample.addVariableToBeDisplayed(
			new DerivedVariable('id3', 'name3', 'datatype1', 'description3', ['id1', 'id2'], {}, [], [], [], '', '')
		);
		...
		expect(Object.keys(rootStore.dataStore.variableStores.sample.referencedVariables)).toHaveLength(3);
		rootStore.dataStore.variableStores.sample.removeVariable('id3');
		expect(Object.keys(rootStore.dataStore.variableStores.sample.referencedVariables)).toHaveLength(0);
	});
	...
	it('creates correct heatmap rows', () => {
		...
		expect(
			rootStore.dataStore.variableStores.sample.childStore.timepoints.every(
				(timepoint) => timepoint.heatmap.length === 3
			)
		).toBe(true);
		expect(
			rootStore.dataStore.variableStores.sample.childStore.timepoints.every(
				(timepoint) => timepoint.primaryVariableId === 'id1'
			)
		).toBe(true);
		rootStore.dataStore.variableStores.sample.removeCurrentVariable('id1');
		expect(
			rootStore.dataStore.variableStores.sample.childStore.timepoints.every(
				(timepoint) => timepoint.heatmap.length === 2
			)
		).toBe(true);
		expect(
			rootStore.dataStore.variableStores.sample.childStore.timepoints.every(
				(timepoint) => timepoint.primaryVariableId === 'id2'
			)
		).toBe(true);
	});
});
