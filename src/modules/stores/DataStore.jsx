...
class DataStore {
	...
	get patientDomains() {
		...
		const domains = this.currentVariables
	...
	get importancePCAScores() {
		...
		const importanceScores = egiVector
			.getColumn(0)
			.map((d, i) => Math.abs(d) + Math.abs(egiVector.getColumn(1)[i]));
	...
	get patientStates() {
		...
		points.forEach((point) => {
			...
			const stateKey = Object.values(pointGroups).find((pointGroup) =>
				pointGroup.pointIdx.includes(idx)
			)?.stateKey;
	...
	setStateLabel = (stateKey, stateLabel) => {
	...
	updatePointGroups = (pointGroups) => {
		this.pointGroups = pointGroups;
		this.numofStates = Object.keys(pointGroups).length;
		this.applyCustomGroups();
	};

	deletePointGroup = (stateKey) => {
		...
		if (!this.pointGroups[NONAME]) {
			...
			const pointIdx1 = this.pointGroups[stateKey].pointIdx,
				pointIdx2 = this.pointGroups[NONAME].pointIdx;
	...
	constructor(rootStore) {
		...
		this.variableStores = {
			// one store for the two different type of blocks (sample/between)
