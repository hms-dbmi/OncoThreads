...
describe('SingleTimepoint', () => {
	...
	beforeAll(() => {
		...
		rootStore.dataStore.variableStores.sample.addVariableToBeDisplayed(
			new OriginalVariable('id1', 'name1', 'NUMBER', 'description1', [], [], mapper1, '', '')
		);
		rootStore.dataStore.variableStores.sample.addVariableToBeDisplayed(
			new OriginalVariable('id2', 'name2', 'STRING', 'description1', [], [], mapper2, '', '')
		);
	});
	...
	it('sorts correctly', () => {
		...
		expect(timepoint.heatmapOrder.slice()).toEqual(sortOrder.map((d) => d.patient));
		timepoint.sort('id1');
		expect(timepoint.heatmapOrder.slice()).toEqual(sortOrder.map((d) => d.patient).reverse());
	});
	it('groups correctly', () => {
		timepoint.group('id2');
