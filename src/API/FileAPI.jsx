...
class FileAPI {
	...
	areProfiled(genes, profileId, callback) {
		const profiledDict = {};
		const profile = this.localFileLoader.molecularProfiles.filter((d) => d.molecularProfileId === profileId)[0];
		let key = '';
		if (
	...
	getMolecularValues(profileId, entrezIDs, callback) {
		...
		entrezIDs.forEach((d) => {
			if (this.localFileLoader.profileData.get(profileId).has(d.entrezGeneId)) {
				returnArr = returnArr.concat(this.localFileLoader.profileData.get(profileId).get(d.entrezGeneId));
			}
		});
		callback(returnArr);
	}
